...
def generate_candidates(tailor,
                        pruning_depth=False,
                        pruning_independent_stages=False,
                        pruning_independent_blocks=False,
    ...
    for stage_k in stage_k_list:
        if stage_k == "reduce_depth" and pruning_depth:
    ...
    for dim_name, skip_stages in skipcode.items():
        if dim_name == "reduce_depth" and pruning_depth:
            continue
    ...
    if "reduce_depth" in stage_k_list and pruning_depth:
        num_k -= 1
    for stage_v_comb in tqdm(stage_v_comb_list):
        ...
        for i, stage_k in enumerate(stage_k_list):
            ...
            if (stage_k != "reduce_depth") or (stage_k == "reduce_depth" and not pruning_depth):
    ...
    for block_type in block_type_list:
        ...
        for i in range(len(supercode_list)):
            ...
            if "reduce_depth" in tailor.stage_vars:
                # keep minimal depth
                if i not in skipcode["reduce_depth"] and block_type == reduced_block_types[i] and pruning_depth:
            ...
            stage_blocks_num.append(stage_num_blocks)
        ...
        if calculate_mode:
            ...
            for i, block_k in enumerate(block_k_list):
                ...
                block_v_list.append(block_v)
                print(f"{block_k}: {block_v}")
            if not pruning_independent_stages:
                num_block_v_comb_list = 1
                for i, block_k in enumerate(block_k_list):
                    offset = 0
                    if block_k == "kernel_size" and pruning_kernel_size:
                        offset = 1
                        continue
                    if pruning_independent_blocks:
                        num_block_v_comb_list *= len(block_v_list[i-offset])*num_blocks
                    else:
                        num_block_v_comb_list *= len(block_v_list[i-offset])**num_blocks
            else:
                num_block_v_comb_list = 1
                ...
                for i, block_k in enumerate(block_k_list):
                    if block_k == "kernel_size" and pruning_kernel_size:
                        ...
                        continue
                    tmp_num = 0
                    for j, stage_num_blocks in enumerate(stage_blocks_num):
                        print(f"# blocks {i} in stage {j}: {stage_num_blocks}")
                        if pruning_independent_blocks:
                            tmp_num += len(block_v_list[i-offset])*stage_num_blocks
                        else:
                            tmp_num += len(block_v_list[i])**stage_num_blocks
                        print(tmp_num)
                    tmp_num = max(1, tmp_num)
                    num_block_v_comb_list *= tmp_num
            
            print(f"# {block_type}: {num_block_v_comb_list}")
            num_block_dict[block_type] = num_block_v_comb_list
            continue
                    
        
        # imrpoved by stage-level independent
        if not pruning_independent_stages:
            # WARN: too high dimension
                ...
                for block_v_comb in stage_block_v_comb_list:
                    ...
                    for i in range(num_k):
                        if block_k_list[i] == "kernel_size" and pruning_kernel_size:
                            ...
                            if "reduce_depth" in stage_k_list and block_type == reduced_block_types[stage_id] and reduce_num and stage_id not in skipcode["reduce_depth"] and pruning_depth:
                                # keep minimal depth
                                assert reduce_num >= 0
    ...
    if calculate_mode:
        block_comb_candidates = 1
        for block_type, num_blocks in num_block_dict.items():
            block_comb_candidates *= num_blocks
