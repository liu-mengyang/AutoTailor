...
class Profiler(object):
    def __init__(self,
        ...
        backend_config["backend_key"] = f"{command_config['backend']}_{command_config['processor_type']}"
    ...
    def generate_and_convert(self,
                             ...
                             enable_fp16=True,
                             enable_int8=False):
        error_save_path = os.path.join(workspace, 'results_gcWorker', 'generate_error.log')
        ...
        try:
            ...
            if convert_configs is None:
                ...
                for config in kernel_configs:
                    ...
                    model_path = os.path.join(self.convertor.save_root, "onnx", ("_".join([kernel_type, mark, id]) + '.onnx'))
                    os.makedirs(os.path.join(self.convertor.save_root, "onnx"), exist_ok=True)
            ...
            save_dir=os.path.join(self.convertor.save_root, 'ncnn', ("_".join([kernel_type, mark])))
            ...
            for id, config_dict in convert_configs.items():
                ...
                if kernel_type == "Mul" and config_dict['config'][0] == config_dict['config'][1]:
                     config_dict['config'][0] = [config_dict['config'][0], config_dict['config'][0]]
                if kernel_type == "Gelu":
                    input_shape = [config_dict['config'][0][0],
                                 config_dict['config'][0][3],
                                 config_dict['config'][0][1] * config_dict['config'][0][2]]
                    input_reshape = [config_dict['config'][0][0],
                                 config_dict['config'][0][3],
                ...
                converted_path_in_table = os.path.join('models/ncnn',("_".join([kernel_type, mark])), model_name + '.ncnn')
                ...
                if gc:
                    ...
                    os.remove(os.path.join(save_dir, (model_name+'_tracing.pnnx.onnx')))
                    os.remove(os.path.join(save_dir, (model_name+'_tracing.pnnx.param')))
                    os.remove(os.path.join(save_dir, (model_name+'_tracing.pnnx.bin')))
                ...
                if index % 20 == 0:
                    save_profiled_results(convert_configs, detail=True, save_path=config_save_path)
            save_profiled_results(convert_configs, detail=True, save_path=config_save_path)
    ...
    def profile(self,
        ...
        if enable_pipeline:
            ...
            gc_worker = gcWorker(self.gc_task_queue,
                                 self.wp_task_queue,
                                 self.backend_config,
                                 self.command_config,
                                 model_save_dir,
            ...
            sp_worker = spWorker(self.wp_task_queue,
                                 self.out_queue,
                                 self.backend_config,
                                 self.command_config,
                                 self.connector,
        ...
        if enable_pipeline:
            ...
            while profiled_index < num_kernels:
                ...
                if converted_index < num_kernels and num_tasks <= 20:
                    ...
                    converted_index += 1
                    logger.info("Generating: " + str(converted_index) + ' | ' + str(num_kernels))
                ...
                if not self.out_queue.empty():
                    ...
                    profiled_index += 1
                    logger.info("Returning: " + str(profiled_index) + ' | ' + str(num_kernels))
                
                    if profiled_index > 0 and profiled_index % 5 == 0 and save_results:
                        # save configs
                        save_profiled_results(profiled_model_configs, detail=True, save_path=profile_save_path)
    ...
    def profile_batch_models(self, model_configs, op_type=None, mode=None):
        ...
        for key, model_config in model_configs.items():
            ...
            if self.backend_config["DEVICE_NAME"] == "localhost": 
                model_path = model_config['converted_model'].split('.ncnn')[0] + "_tracing.ncnn"
            else:
            ...
            if op_type == 'AveragePool' or op_type == 'MaxPool':
            ...
            elif op_type == 'Linear' or op_type == 'LinearMatMul' or op_type == 'GlobalAveragePool' or op_type == 'Transpose' or op_type == 'Reshape' or op_type == 'Softmax' or op_type == 'LayerNormalization':
                input_shape = model_config['config'][1]
            elif op_type == 'Add' or op_type == 'Relu' or op_type == 'ReduceMean' or op_type == 'HardSwish' or op_type == 'HardSigmoid' or op_type == 'BiasAdd' or op_type == "Flatten" or op_type == 'BatchNormalization':
                ...
                if model_config['config'][0] == model_config['config'][1]:
                    input_shape = {"input_1": model_config['config'][0], "input_2": model_config['config'][0]}
                else:
                    input_shape = {"input_1": model_config['config'][0][0], "input_2": model_config['config'][0][1]}
            elif op_type == 'Gelu':
            ...
            elif op_type == "BiasAdd" or op_type == "Transpose" or op_type == "Reshape" or op_type == 'LinearMatMul':
                    if len(input_shape)==4:
                        self.command_config["HW0"] = input_shape[2]
                        self.command_config["HW1"] = input_shape[3]
                        self.command_config["CIN"] = input_shape[1]
                        
                        configs['shape'] = f'[{input_shape[3]},{input_shape[2]},{input_shape[1]},{input_shape[0]}]'
                        ...
                        if op_type == "LinearMatMul":
                            ...
                            configs["shape"] = f"{input_shape[0]},{input_shape[1]},{input_shape[2]}"
                        else:
                            self.command_config["HW0"] = input_shape[1]
                            self.command_config["HW1"] = input_shape[2]
                            self.command_config["CIN"] = input_shape[0]
                            
                            configs['shape'] = f'[{input_shape[1]},{input_shape[2]},{input_shape[0]}]'
    ...
    def profile_model(self, model, input_shape, model_name=None, op_type=None, ret_convert_time=False):
        model = model.eval()
        if model_name == None:
            model_name='test'
        with torch.no_grad():
            ...
            if self.backend_config["backend_name"] == "ncnn":
                ...
                if isinstance(input_shape, dict):
                    ...
                    for k,v in input_shape.items():
                        if op_type == "MatMul":
                            ...
                            if len(str_shapes) == 0:
                                ...
                                str_shapes += f',[{c}]'
                    configs['shape'] = str_shapes
                elif op_type == "BiasAdd" or op_type == "Transpose" or op_type == "Reshape":
                    if len(input_shape)==4:
                        self.command_config["HW0"] = input_shape[2]
                        self.command_config["HW1"] = input_shape[3]
                        self.command_config["CIN"] = input_shape[1]
                        
                        configs['shape'] = f'[{input_shape[2]},{input_shape[3]},{input_shape[1]},{input_shape[0]}]'
                        # if op_type == "Gelu":
                        #     self.command_config["HW0"] = input_shape[2]
                        #     self.command_config["HW1"] = input_shape[3]
                        #     self.command_config["CIN"] = input_shape[1]
                    ...
                    self.command_config["CIN"] = input_shape[2]
                    print(input_shape)
                    configs['shape'] = f'[{input_shape[3]},{input_shape[1]},{input_shape[2]}]'
                    print(configs["shape"])
                elif len(input_shape)==4:
                    self.command_config["HW0"] = input_shape[2]
                    self.command_config["HW1"] = input_shape[3]
