...
def measure(serial, item, root, model, checker, converter):
    ...
    identity = {'id': identifier, 'serial': serial, 'torchscript_sha256': item['torchscript_sha256'],
        ...
        'cooldown_seconds_per_run': 10, 'runtime': 'NCNN 20240102, Android arm64 CPU'}
    ...
    args = ['./bench', '100', '1', '2', '-1', '1', 'model_name=model', f'shape=[{res},{res},3]']
...
def main():
    ...
    serials = os.getenv('AE_PHONE_SERIALS', '').split()
    if len(serials) != 2 or len(set(serials)) != 2:
        p.error('The authors must configure two distinct phones in AE_PHONE_SERIALS on the phone server.')
