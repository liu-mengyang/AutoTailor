...
class Extractor(object):
    ...
    def pre_build(self, block_building=False, skip_recount=False):
        last_reduce_depth = None
        for code in self.code_list:
            if skip_recount:
                curr_reduce_depth = code.get("reduce_depth", None)
                if curr_reduce_depth != last_reduce_depth:
                    self.tailor.tir._depth_dirty = True
                last_reduce_depth = curr_reduce_depth
            self.tailor.transform(code)
            self.tailor.tir.pre_build(
                self.built_op_dict,
                block_building=block_building,
                skip_recount=skip_recount)
