...
def _canon_key(d):
    """Stable hashable key for a candidate dict, for O(1) set-based dedup
    instead of O(n^2) `if d not in list` membership (which made ViT's ~531k
    candidate space effectively hang generate_candidates)."""
...
def generate_candidates(tailor,
                        pruning_kernel_size=False,
                        maximal_depth=False,
                        pruning_block_comb=False,
                        single_block=False):
    # num_workers = 1
    # num_workers = 32
    ...
    for block_type in block_type_list:
        ...
        for i in range(len(supercode_list)):
            ...
            if pruning_block_comb:
                stage_block_v_comb_list = []
                for stage_id in range(stage_num_blocks):
                    for block_v in block_v_list:
                        # construct config
                        
                        stage_block_v_comb_list.append()
            elif single_block:
                # Memory/scale-safe enumeration: vary ONE block at a time (others
                # held at the supercode/reference value) instead of the full
                # cross-product. An op's key depends only on its own block's dims
                # (residual blocks keep shape), so this yields the same unique
                # op-key set with stage_num_blocks*|per-block combos| candidates
                # instead of |per-block combos|^stage_num_blocks (12*3 vs 3^12 for ViT).
                ref = tuple(max(bv) for bv in block_v_list)
                per_block = list(it.product(*block_v_list))
                _seen = set()
                stage_block_v_comb_list = []
                for p in range(stage_num_blocks):
                    for combo in per_block:
                        comb = tuple(x for b in range(stage_num_blocks)
                                     for x in (combo if b == p else ref))
                        if comb not in _seen:
                            _seen.add(comb)
                            stage_block_v_comb_list.append(comb)
            else:
        ...
        for stage_id, stage_block_v_comb_list in enumerate(block_v_comb_list):
            for block_v_comb in stage_block_v_comb_list:
                ...
                _fk = _canon_key(final_code)
                if _fk not in _sub_seen:
                    _sub_seen.add(_fk)
...
def prebuild_worker(worker_id,
    ...
    for i, stage_block_candidate in enumerate(worker_task):
        ...
        task_codes.append(code)
    extractor.set_code_list(task_codes)
    extractor.pre_build(block_building=block_building, skip_recount=skip_recount)
...
def prebuild_worker_flat(worker_id,
                         extractor,
                         glob_candidates,
                         stage_block_slice,
                         queue,
                         skip_recount=False):
    """Flat variant: worker owns a stage_block slice and iterates all glob candidates
    internally, eliminating the need to re-spawn for each glob round."""
    extractor.clear_op_dict()
    total = len(glob_candidates) * len(stage_block_slice)
    print(f"Worker {worker_id} created with {total} tasks "
          f"({len(glob_candidates)} globs × {len(stage_block_slice)} stage_block)")
    task_codes = []
    for glob_candidate in glob_candidates:
        code_base = {**glob_candidate}
        for sb_candidate in stage_block_slice:
            code = copy.deepcopy(code_base)
            for var_candidate in sb_candidate:
                if isinstance(var_candidate, tuple):
                    for block_type_candidate in var_candidate:
                        for var_key, var_value in block_type_candidate.items():
                            code[var_key] = var_value
            ...
            task_codes.append(code)
    extractor.set_code_list(task_codes)
    extractor.pre_build(block_building=True, skip_recount=skip_recount)
    queue.put(extractor.built_op_dict)
    print(f"Worker {worker_id} done")
    # if not block_level:
    #     return extractor.built_op_dict
