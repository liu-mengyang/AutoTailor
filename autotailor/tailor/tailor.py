...
class Tailor(object):
    ...
    def one_step_sample(self, code):
        """One-step (Hamming-1) neighbor sampling.

        Return a copy of `code` with EXACTLY ONE tunable dimension changed by one
        value: either a single stage's reduce_depth or a single block's expand_ratio.
        Resolution and every other dimension are left untouched, and positions fixed
        by `self.skipcode` (skipped stages/blocks) are never mutated. Used by the
        "onestep" sampling strategy (anchor + one-step neighbors) to maximise
        intra-iteration computation sharing at sampling time. If no mutable position
        exists, the unchanged copy is returned.
        """
        new_code = copy.deepcopy(code)

        # Normalise skip indices to non-negative stage ids (transform() does the same).
        skip = {}
        for dim_name, skipids in self.skipcode.items():
            skip[dim_name] = set((s + self.num_stage) if s < 0 else s for s in skipids)

        # Enumerate every one-step option: (kind, location, new_value).
        options = []
        # Stage-level dims (e.g. reduce_depth) — skipped stages are fixed to max.
        for stage_k, stage_v in self.stage_vars.items():
            if "skipped" in stage_k or stage_k not in new_code:
                continue
            for i in range(self.num_stage):
                if i in skip.get(stage_k, set()):
                    continue
                cur = new_code[stage_k][i]
                for v in stage_v:
                    if v != cur:
                        options.append(("stage", (stage_k, i), v))
        # Block-level dims (e.g. expand_ratio) — blocks in skipped stages are fixed.
        for block_type, block_vars in self.block_vars.items():
            if block_type not in new_code:
                continue
            for block_k, block_v in block_vars.items():
                for i in range(self.num_stage):
                    if i in skip.get(block_type, set()):
                        continue
                    for j in range(self.num_dynamic_block_dict[block_type][i]):
                        cur = new_code[block_type][block_k][i][j]
                        for v in block_v:
                            if v != cur:
                                options.append(("block", (block_type, block_k, i, j), v))

        if not options:
            return new_code

        kind, loc, val = random.choice(options)
        if kind == "stage":
            stage_k, i = loc
            new_code[stage_k][i] = val
        else:
            block_type, block_k, i, j = loc
            new_code[block_type][block_k][i][j] = val
        return new_code
