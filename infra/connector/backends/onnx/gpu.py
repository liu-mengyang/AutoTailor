...
class OnnxGPULatencyParser(BaseParser):
    def __init__(self):
        self.nodes = []
...
class OnnxGPUProfiler(OnnxProfiler):
    use_gpu = True


class OnnxGPUBackend(OnnxBackend):
    parser_class = OnnxGPULatencyParser
    profiler_class = OnnxGPUProfiler
