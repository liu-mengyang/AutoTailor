...
class TFLiteGPULatencyParser(BaseParser):
    ...
    def _parse_comp_time(self, content):
        ...
        comp_time_regex = r'Timings \(microseconds\): count=[\d.e-]+ first=[\d.e-]+ curr=[\d.e-]+ min=[\d.e-]+ max=[\d.e-]+ avg=([\d.\+e-]+) std=([\d.\+e-]+)'
        ...
        for line in content.splitlines():
            ...
            if match:
                comp_avg = float(match[1]) / 1000
                comp_std = float(match[2]) / 1000
                match_flag = 1
